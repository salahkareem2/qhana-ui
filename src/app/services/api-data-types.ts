...
export interface CollectionApiObject extends ApiObject {
    collectionSize: number;
    items: ApiLink[];
}

export function isCollectionApiObject(obj: any): obj is CollectionApiObject {
    if (!isApiObject(obj)) {
        return false;
    }
    const self = obj.self;
    const hasCollectionRel: boolean = (
        (self.resourceType === "collection") ||
        (self.resourceType === "page") ||
        self.rel.some(rel => rel === "collection" || rel === "page")
    );
    if (!hasCollectionRel) {
        return false;
    }
    const hasCollectionAttrs: boolean = (
        (obj as CollectionApiObject)?.items != null
        && Array.isArray((obj as CollectionApiObject)?.items)
        && (obj as CollectionApiObject)?.items.every(link => isApiLinkBase(link))
        && (obj as CollectionApiObject)?.collectionSize != null
        && Number.isInteger((obj as CollectionApiObject)?.collectionSize)
    );
    return hasCollectionAttrs;
}

export interface PageApiObject extends CollectionApiObject {
    collectionSize: number;
    items: ApiLink[];
    page: number;
}

export function isPageApiObject(obj: any): obj is PageApiObject {
    if (!isCollectionApiObject(obj)) {
        return false;
    }
    return (obj as PageApiObject)?.page != null && Number.isInteger((obj as PageApiObject)?.page);
}
